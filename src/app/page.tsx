...
export default function SetupPage() {
  ...
  const MET_DATA = [
    ...
    { id: 'MET 2', name: 'Conduct Casualty Treatment', events: ['HSS-MBN-6001', 'HSS-STP-5001', 'HSS-FRSS-4001', 'HSS-SVCS-3507'] },
    { id: 'MET 3', name: 'Conduct Temp Casualty Holding', events: ['HSS-SVCS-3502', 'HSS-SVCS-3507'] },
    { id: 'MET 4', name: 'Conduct Casualty Evacuation', events: ['HSS-SVCS-3401', 'HSS-PET-4701'] },
    { id: 'MET 5', name: 'Conduct Mass Casualty Operations', events: ['HSS-SVCS-3701', 'HSS-OPS-7001'] },
    { id: 'MET 6', name: 'Conduct Dental Services', events: ['HSS-DENT-3001', 'HSS-DENT-3002'] },
    { id: 'MET 7', name: 'Conduct Medical Regulating', events: ['HSS-OPS-6001', 'HSS-PET-4701'] }
  ];
  ...
  const generateName = async () => {
    ...
    } catch (e) { setExerciseName("Operation Iron Triage"); }
  ...
  return (
    <div className="max-w-6xl mx-auto p-8 bg-slate-50 min-h-screen text-slate-900 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-4 rounded-xl border-2 border-slate-200 col-span-2">
          ...
          <div className="flex space-x-2">
            ...
            <button onClick={generateName} className="bg-blue-600 text-white px-4 rounded font-black text-xs hover:bg-blue-700 transition">{isGenerating ? "..." : "AI GEN"}</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <section className="bg-white p-6 rounded-2xl shadow-xl border-2 border-slate-200">
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
            {MET_DATA.map(met => (
              <div key={met.id} className="border-2 border-slate-100 rounded-lg">
                <div className="flex items-center p-3 bg-slate-50 hover:bg-blue-50 transition cursor-pointer">
                  ...
                  <div className="font-black">{expandedMET === met.id ? '▾' : '▸'}</div>
                </div>
                {expandedMET === met.id && (
                  <div className="p-4 bg-white border-t-2 border-slate-50 space-y-2">
                    {met.events.map(event => <div key={event} className="text-[10px] text-slate-600 border-l-2 border-blue-200 pl-3 font-mono">{event}</div>)}
                  </div>
                )}
              </div>
            ))}
        ...
        <section className="bg-white p-6 rounded-2xl shadow-xl border-2 border-slate-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            {FOOTPRINT.map(s => (
              <label key={s} className="flex items-center space-x-2 p-2 bg-slate-50 rounded text-[10px] font-bold uppercase cursor-pointer hover:bg-emerald-50 transition">
      ...
      <Link href="/tactical">
        <button className="mt-12 w-full bg-slate-900 text-white font-black py-8 rounded-3xl shadow-2xl hover:bg-black transition text-3xl uppercase tracking-tighter border-b-[12px] border-slate-700">Set Casualty Flow →</button>
      </Link>
    </div>
  );
}